...
def x_control_xy_correlation(x, y, L, lambda_xy):
    Lx = np.einsum('ij,jk->ik', L, x)
    xTLx = np.einsum('ij,ij->j', x, Lx).reshape(1,1)
    ...
    e_xy = yTx - 1
    alignment_term = e_xy * (xTLx*yTx - yTLx)
    V_xy = e_xy**2/2
    b_xy = max(0, (alignment_term + lambda_xy*V_xy)) / ((yTy + 1e-6) * (e_xy**2 + 1e-6))
    return b_xy

def y_control_xy_correlation(x, y, L, lambda_yy):
    LTy = np.einsum('ji,jk->ik', L, y)
    yTLTy = np.einsum('ij,ij->j', y, LTy).reshape(1,1)
    yTLx = np.einsum('ij,ij->j', x, LTy).reshape(1,1)
    yTx = np.einsum('ij,ij->j', y, x).reshape(1,1)
    xTx = np.einsum('ij,ij->j', x, x).reshape(1,1)
    e_xy = yTx - 1
    alignment_term = e_xy * (yTLTy*yTx - yTLx)
    V_xy = e_xy**2/2
    b_y = max(0, (alignment_term + lambda_yy*V_xy)) / ((xTx + 1e-6) * (e_xy**2 + 1e-6))
    return b_y

def dynamics(L, x, y, lambda_x, lambda_xy, lambda_y):
    x = x.reshape(-1,1)
    y = y.reshape(-1,1)
    ...
    b_xy = x_control_xy_correlation(x, y, L, lambda_xy)
    b_y = y_control_xy_correlation(x, y, L, lambda_y)
    ...
    L_x = graph_loss - lambda_Lx*x2/2 + b_x*e_x**2/2 + b_xy*e_xy**2/2
    L_y = graph_loss - lambda_Ly*y2/2 + b_y*e_xy**2/2
...
def step(L, x, y, lr, lambda_x, lambda_xy, lambda_y):
    
    # Compute gradients
    (fx, fy), aux = dynamics(
        L, x, y, lambda_x, lambda_xy, lambda_y
    )
    ...
    grad_norms = {
        ...
        '$||u_xy||$': np.linalg.norm(aux['u_xy']),
        '$||u_y||$': np.linalg.norm(aux['u_y']),
    }

    return (x_new, y_new), aux, grad_norms

def simulate_dynamics(L, x0, y0, lr, lambda_x, lambda_xy, lambda_y, n_steps):
    ...
    for _ in tqdm(range(n_steps), desc="Simulating dynamics", leave=True):
        (x, y), aux, grad_norms = step(
            L, x, y, lr, lambda_x, lambda_xy, lambda_y
        )
        trajectory.append(
            (x.copy(), y.copy())
        )
...
def visualize_metrics(t, cs_v, cs_u, eigval, aux_arrays, grad_norm_arrays):    
    ...
    e_xy_t = aux_arrays['e_xy']
    ny_t = aux_arrays['y2']**0.5
    plt.plot(t1, np.abs(e_x_t), label=r'$e_x$')
    plt.plot(t1, np.abs(e_xy_t), label=r'$e_{xy}$')
    plt.plot(t1, ny_t, label=r'$||y||_2$')
    ...
    b_xy_t = aux_arrays['b_xy']
    b_y_t = aux_arrays['b_y']
    plt.plot(t1, b_x_t, label=r'$b_x$')
    plt.plot(t1, b_xy_t, label=r'$b_{xy}$')
    plt.plot(t1, b_y_t, label=r'$b_y$')
    ...
    plt.subplot(2, 4, 8)
    for key, values in grad_norm_arrays.items():
        if key in ['$||\\bar{f}_x||$', '$||\\bar{f}_y||$', '$||u_x||$', '$||u_{xy}||$', '$||u_y||$']:
            plt.plot(t1, values, label=key)
    plt.title('Gradient Norms')
    plt.legend()
    plt.xlabel('Time Step')
...
def main(n, delta, lr, lambda_x, lambda_xy, lambda_y, n_steps):
    ...
    t, x_t, y_t, aux_arrays, grad_norm_arrays  = simulate_dynamics(
        L, x0, y0, lr, lambda_x, lambda_xy, lambda_y, n_steps)
...
if __name__ == "__main__":
    ...
    lambda_y = 10
    n_steps = 20000
    main(n, delta, lr, lambda_x, lambda_xy, lambda_y, n_steps)