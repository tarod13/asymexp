...
def dynamics(L, x, y, lambda_x, lambda_xy):
    x = x.reshape(-1,1)
    y = y.reshape(-1,1)
    ...
    b_xy = control_xy_correlation(x, y, L, lambda_xy)
    ...
    L_y = graph_loss - lambda_Ly*y2/2 + b_xy*e_xy**2/2
...
def simulate_dynamics(L, x0, y0, lr, lambda_x, lambda_xy, n_steps):
...
def main(n, delta, lr, lambda_x, lambda_xy, n_steps):
    ...
    t, x_t, y_t, aux_arrays, grad_norm_arrays  = simulate_dynamics(
        L, x0, y0, lr, lambda_x, lambda_xy, n_steps)
...
if __name__ == "__main__":
    ...
    lambda_xy = 10
    n_steps = 20000
    main(n, delta, lr, lambda_x, lambda_xy, n_steps)