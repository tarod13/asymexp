...
This file contains common usage examples for the `run_analysis.sh` script.

## Portal Environment Analysis (Experiment 01)

### Basic run with default parameters
```bash
./run_analysis.sh portal
```

### Generate more environments and eigenvectors
```bash
./run_analysis.sh portal --num-envs 50 --k 40 --num-portals 15
```

### Custom output directory
```bash
./run_analysis.sh portal --output-dir ./results/portal_custom --seed 123
```
...
### Basic run with default parameters
```bash
./run_analysis.sh door
```

### More doors and environments
```bash
./run_analysis.sh door --num-doors 20 --num-envs 100
```

### Enable logarithmic scale for hitting time visualization
```bash
./run_analysis.sh door --num-doors 10 --log-scale
```
...
1. **View all options**: Run `./run_analysis.sh --help` to see all available options
