...
def create_update_function(encoder, args):
    ...
    def update_encoder(
        ...
        # Compute separate gradient norms for encoder and EMA parameters
        ...
        ema_grads = [
            grads['lambda_real'], grads['lambda_imag'],
            grads['norm_x_ema'], grads['norm_y_ema'], grads['phase_xy_ema'],
            jnp.ravel(grads['corr_xy_real_ema']), jnp.ravel(grads['corr_xy_imag_ema'])
        ]
        ema_grads_flat = [jnp.ravel(g) for g in ema_grads]
        ema_grads_vector = jnp.concatenate(ema_grads_flat)
        ema_grad_norm = jnp.linalg.norm(ema_grads_vector)

        aux['grad_norm'] = encoder_grad_norm
        aux['ema_grad_norm'] = ema_grad_norm

        # Clip gradients independently
        max_norm = 1.0
        encoder_clip_factor = max_norm / jnp.maximum(encoder_grad_norm, max_norm)
        ema_clip_factor = max_norm / jnp.maximum(ema_grad_norm, max_norm)

        grads = {
            'encoder': jax.tree_util.tree_map(lambda g: g * encoder_clip_factor, grads['encoder']),
            'lambda_real': grads['lambda_real'] * ema_clip_factor,
            'lambda_imag': grads['lambda_imag'] * ema_clip_factor,
            'norm_x_ema': grads['norm_x_ema'] * ema_clip_factor,
            'norm_y_ema': grads['norm_y_ema'] * ema_clip_factor,
            'phase_xy_ema': grads['phase_xy_ema'] * ema_clip_factor,
            'corr_xy_real_ema': grads['corr_xy_real_ema'] * ema_clip_factor,
            'corr_xy_imag_ema': grads['corr_xy_imag_ema'] * ema_clip_factor,
        }
