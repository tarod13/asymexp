...
class LaplacianEncoderTrainer(Trainer, ABC):    # TODO: Handle device
    ...
    def _compute_metrics_tabular(self, params, metrics_dict):
        ...
        cs, sim = self.compute_cosine_similarity(
        ...
        self.train_info['cos_sim'] = np.array([jax.device_get(cs)])[0]
    ...
    def compute_cosine_similarity(self, params_encoder, batch_size=32):
        ...
        # if self.is_permute_phase:
        #     approx_eigvec = self.permute_representations(approx_eigvec)
    ...
    def compute_cosine_similarity_simple(self, params_encoder):
        ...
        # Convert to array
        similarities = jnp.array(similarities)
        permuted_similarities = jnp.array(permuted_similarities)
        # print(f'Similarities: {similarities}')
    ...
    def plot_single_eigenvector(self, states, eigenvector_id, eigenvector, grid, vmin, vmax):
        ...
        # Save figure
        run_id = self.logger.id if (self.use_wandb and self.logger is not None) else self._date_time
        fig_path = f'./results/visuals/{self.env_name}/learned_eigenvector_{eigenvector_id}_{run_id}.pdf'
